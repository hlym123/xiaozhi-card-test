...
void XiaozhiCardBoard::StartUp()
{
    ...
    int brightness = 0;
    bool charging = false;
    lv_obj_t *scr_low_battery = nullptr;
    lv_obj_t *label_tip = nullptr;
    lv_obj_t *label_bat_info = nullptr;
    while (1) {
        if (tick % 20 == 0) {
...
void XiaozhiCardBoard::Disable4GTask(void *param) 
{
    ...
    for (uint8_t i = 0; i< 5; i++) {
        if (modem->PowerOff()) {
            printf("\n4g power off success\n");
            //delete modem; // 引脚释放导致 ML307 模组重启？？？
            break;
        } else {
            printf("\n4g power off failed\n");
...
void XiaozhiCardBoard::Shutdown()
{
    ...
    if (charger_->GetChargeState()) {
        ...
        // app.Alert("充电中不能关机", "", "sad", Lang::Sounds::P3_CLICK);
        vTaskDelay(pdMS_TO_TICKS(1000));  
        // app.DismissAlert();
    ...
    if (network_type == NetworkType::ML307) {
        ...
        for (int i = 0; i < 5; i++) {
            if (ml307_board.PowerOff()) {
                printf("\nML307 Power Off success\n");
                modem_powered_on_ = false;
                break; 
            } else {
                printf("\nML307 Power Off failed\n");
                vTaskDelay(pdMS_TO_TICKS(100));
            }
        }
    } else if (network_type == NetworkType::WIFI) {
        printf("\nNetworkType::WIFI\n");
        // if (modem_shutdown_sem) {
        //     if (xSemaphoreTake(modem_shutdown_sem, pdMS_TO_TICKS(3000)) == pdTRUE) {
        //         ESP_LOGI(TAG, "modem_shutdown_task finished");
        //     } else {
        //         ESP_LOGW(TAG, "modem_shutdown_task timeout");
        //     }
        //     vSemaphoreDelete(modem_shutdown_sem);
        //     modem_shutdown_sem = nullptr;
        // }
...
void XiaozhiCardBoard::BoardEventTask(void* param) 
{
    ...
    while (1) {
        ...
        // 配置打开 Invoke panic handler on Task Watchdog timeout 
        // vTaskDelay(pdMS_TO_TICKS(5000));  // 看门狗超时触发系统复位测试 
        // printf("\n============================\n");
    }
}
...
XiaozhiCardBoard::XiaozhiCardBoard() : DualNetworkBoard(ML307R_PIN_TX, ML307R_PIN_RX, ML307R_PIN_RST, ML307R_PIN_PWR, ML307R_PIN_DTR, ML307R_PIN_WAKEUP, 20*1024),
         user_button_(USER_BUTTON_GPIO, false, 2000, 200)
{
